...
{ 
	TestSuite testSuite;
  	...
  	testSuite.require("Création d'un infirmier", 0.5, [](){
	...
	testSuite.require("Modification des attributs d'un infirmier", 0.5, [](){
		...
		assert(i.getPrenom() == "Florence", "Erreur sur la modification du prénom");
	...
	testSuite.require("Gestion des chambres", 0.5, [](){
		...
		assert(i.getChambre()[0] == "101", "Erreur: chambre 101 non trouvée");
		assert(i.getChambre()[1] == "102", "Erreur: chambre 102 non trouvée");
	});
	...
	testSuite.require("Suppression de chambres", 0.5, [](){
		...
		assert(i.getChambre()[0] == "102", "Erreur: mauvaise chambre supprimée");
		i.retirerChambre("103");
		assert(i.getChambre().size() == 1, "Erreur: suppression d'une chambre inexistante");
	});

	testSuite.require("Gestion des heures travaillées", 0.5, [](){
		Infirmier i("Nightingale", "Florence", 15.0);
		i.ajouterHeuresTravaillees(5);
		assert(i.getHeuresTravaillees() == 5, "Erreur sur l'ajout des heures travaillées");
		i.ajouterHeuresTravaillees(3);
		assert(i.getHeuresTravaillees() == 8, "Erreur sur l'ajout cumulé des heures travaillées");
	});
	...
	testSuite.require("Calcul du salaire", 0.5, [](){
		...
		assert(i.getHeuresTravaillees() == 0, "Erreur: heures travaillées non réinitialisées après le calcul du salaire");
	});

	testSuite.require("Affichage des détails de l'infirmier", 1, []() {
		Infirmier i("Nightingale", "Florence", 15.0);
		i.ajouterHeuresTravaillees(5);
		i.ajouterChambre("101");
		i.ajouterChambre("102");
	...
	testSuite.require("Création d'une spécialité par défaut", 0.5, [](){
		Specialite s;
		assert(s.getDomaine() == "", "Erreur: domaine par défaut incorrect");
		assert(s.getNiveau() == 0, "Erreur: niveau par défaut incorrect");
	});
	
	testSuite.require("Création d'une spécialité avec paramètres", 0.5, [](){
		Specialite s("Cardiologie", 5);
		assert(s.getDomaine() == "Cardiologie", "Erreur sur le domaine");
		assert(s.getNiveau() == 5, "Erreur sur le niveau");
	});

	testSuite.require("Modification des attributs d'une spécialité", 0.5, [](){
		Specialite s;
	...
	testSuite.require("Affichage des détails d'une spécialité", 0.5, [](){
		Specialite s("Pédiatrie", 3);
		ostringstream oss;
	...
	testSuite.require("Création d'un médecin avec nom uniquement", 0.5, [](){
		Medecin m("Fauci");
		assert(m.getNom() == "Fauci", "Erreur: nom incorrect");
		assert(m.getSpecialite() == nullptr, "Erreur: spécialité initiale non nulle");
	});

	testSuite.require("Création d'un medecin avec parametres", 0.5, [](){
	...
	testSuite.require("Modification des attributs d'un médecin", 0.5, [](){
		Medecin m("Dupont",  nullptr, 100000.0);
		Specialite s("Neurologie", 4);
		m.setNom("Fauci");
		m.setSpecialite(&s);
		assert(m.getNom() == "Fauci", "Erreur: modification du nom");
		assert(m.getSpecialite() == &s, "Erreur: modification de la spécialité");
	});	

	testSuite.require("Calcul du salaire sans spécialité", 0.5, [](){
		Medecin m("Fauci", nullptr, 120000.0 );
		assert(m.calculerSalaire() == 120000.0, "Erreur: calcul du salaire sans spécialité incorrect");
	});

	testSuite.require("Calcul du salaire avec spécialité", 0.5, [](){
		Specialite s("Pédiatrie", 3);
		Medecin m("Fauci", &s, 80000.0);
		assert(m.calculerSalaire() == 240000.0, "Erreur: calcul du salaire avec spécialité incorrect");
	});

	testSuite.require("Affichage des détails d'un médecin", 1, []() {
	...
	testSuite.require("Création d'un hôpital avec un nom", 0.5, [](){
		HopitalPoly h("Hopital Sainte-Justine");
		assert(h.getNom() == "Hopital Sainte-Justine", "Erreur: nom incorrect");
		assert(h.getMedecins().empty(), "Erreur: liste de médecins non vide à la création");
		assert(h.getInfirmiers().empty(), "Erreur: liste d'infirmiers non vide à la création");
	});

	testSuite.require("Ajout d'un médecin à l'hôpital", 0.5, [](){
		HopitalPoly h("Hopital Sacré-Coeur");
		Medecin m("Fauci");
		h.ajouterMedecin(&m);
		assert(h.getMedecins().size() == 1, "Erreur: médecin non ajouté");
		assert(h.getMedecins()[0] == &m, "Erreur: mauvais médecin dans la liste");
	});

	testSuite.require("Suppression d'un médecin de l'hôpital", 0.5, [](){
	...
	testSuite.require("Ajout d'un infirmier à l'hôpital", 0.5, [](){
		HopitalPoly h("Hopital Général Juif");
		Infirmier i("Florence", "Nightingale", 20.0);
		h.ajouterInfirmier(&i);
		assert(h.getInfirmiers().size() == 1, "Erreur: infirmier non ajouté");
		assert(h.getInfirmiers()[0] == &i, "Erreur: mauvais infirmier dans la liste");
	});

	testSuite.require("Suppression d'un infirmier de l'hôpital", 0.5, [](){
		HopitalPoly h("Hopital de Montréal pour Enfants");
		Infirmier i("Florence", "Nightingale", 25.0);
		h.ajouterInfirmier(&i);
		h.supprimerInfirmier(&i);
		assert(h.getInfirmiers().empty(), "Erreur: infirmier non supprimé");
	});

	testSuite.require("Recherche d'un médecin dans l'hôpital", 0.5, [](){
		HopitalPoly h("Hopital Charles-LeMoyne");
		Medecin m("Smith");
		h.ajouterMedecin(&m);
		assert(h.chercherMedecin(&m), "Erreur: médecin non trouvé");
		Medecin m2("Johnson");
		assert(!h.chercherMedecin(&m2), "Erreur: faux positif pour médecin inexistant");
	});

	testSuite.require("Recherche d'un infirmier dans l'hôpital", 0.5, [](){
		HopitalPoly h("Hopital de Verdun");
		Infirmier i("Florence", "Nightingale", 30.0);
		h.ajouterInfirmier(&i);
		assert(h.chercherInfirmier(&i), "Erreur: infirmier non trouvé");
		Infirmier i2("Mary", "Seacole", 30.0);
		assert(!h.chercherInfirmier(&i2), "Erreur: faux positif pour infirmier inexistant");
	});

	testSuite.require("Affichage de l'hôpital", 2, []() {
		...
		string expectedOutput = 
			...
			"Niveau: 6\n"
			"Salaire: 540000 $\n";
		assert(output == expectedOutput, "Erreur: le format de l'affichage de l'hôpital est incorrect");
	});
