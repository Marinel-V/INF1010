...
ListeMedecin() {}
    ...
    void ajoutermedecin(T *medecin)
    {
        if (dynamic_cast<Z*>(medecin) || dynamic_cast<U*>(medecin))
        {
            heapMedecin_.push_back(medecin);
            push_heap(heapMedecin_.begin(), heapMedecin_.end(), ComparerSalaire<T>());
        }
    ...
    {
        return accumulate(heapMedecin_.begin(), heapMedecin_.end(), 0, Medecin::calculerSalaire());
    }
